        ...
        <div className='grid grid-cols-2'>
          <div className='col-span-1 grid grid-cols-12 gap-1'>
            <div className='col-span-9 relative overflow-hidden'>
              <img src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.collectionName}/${item?.id}/${item?.decor[0]}`} className='w-full h-full' />
            ...
            <div className='col-span-3 flex flex-col gap-1'>
              <img src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.collectionName}/${item?.id}/${item?.decor[1]}`} className='w-full h-full' />
              <img src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.collectionName}/${item?.id}/${item?.decor[2]}`} className='w-full h-full' />
              <img src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.expand?.texture_ref?.collectionName}/${item?.expand?.texture_ref?.id}/${item?.expand?.texture_ref?.poster}`} className='w-full h-full' />
            </div>
          </div>
          <div className='col-span-1 px-4'>
            <h3 className='text-2xl font-bold text-primary capitalize'>{item?.expand?.brand_ref[0].name} {item?.code}</h3>
            ...
            <p className='text-sm  text-gray-400'>Core: {item?.expand?.core_ref?.name} | Surface: {item?.expand?.surface_ref?.name}</p>
            <p className='text-sm  text-gray-400'>Finish: {item?.expand?.finish_ref?.name} | Design: {item?.expand?.design_ref?.name}</p>
            <p className='text-sm  text-gray-400'>Texture: {item?.expand?.texture_ref?.name} | Grain: {item?.expand?.surface_ref?.name}</p>
            ...
            <div className='flex items-center text-center mt-2'>
              ...
              <a href={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.collectionName}/${item?.id}/${item?.attachment}`} download className='flex border  border-gray-400 px-2 py-1 text-primary text-xs font-semibold align-center items-center text-center bg-white rounded-lg'>
                PDF Documents
              </a>
            </div>
          </div>
      ...
      <div className='w-full px-6 py-2 grid grid-cols-12 gap-x-4'>
        ...
        <div className='col-span-7'>
          <div className='flex gap-3 items-center'>
            <button onClick={() => setCunter(cunter + 1)} className='flex items-center justify-center border w-8 h-8 p-1 bg-white  border-gray-400/70 rounded-md'>
              <FaPlus />
            </button>
          ...
          <div className='mt-2'>
            ...
            <div className='bg-gray-100 border border-gray-400/70 p-2 rounded-lg'>
              <div className='flex gap-2'>
                <span className={`cursor-pointer flex items-center justify-center border w-8 h-8 p-1 bg-white hover:bg-primary border-primary text-black hover:text-white ${show == 1 && '!bg-primary text-white'}`} onClick={() => setShow(1)}>{thickness[0]?.thickness}</span>
                <span className={`cursor-pointer flex items-center justify-center border w-8 h-8 p-1 bg-white hover:bg-primary border-primary text-black hover:text-white ${show == 0 && '!bg-primary text-white'}`} onClick={() => setShow(0)}>{thickness[1 ]?.thickness}</span>
              </div>
