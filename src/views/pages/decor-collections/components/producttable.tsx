...
export default function Producttable({item, setOpened, opened, setPupitem}:any) {
    ...
    useEffect(()=>{
        let config = {
            ...
            url: `https://shop.xcuts.co.uk/api/collections/thickness/records?filter=(decor_ref='${item?.id}')`,
            headers: { }
          };
    ...
    <tr className='border-t '>
                  <td>
                    <div className='text-center w-16 h-16 flex items-center justify-center rounded-full overflow-hidden'>
                        <img className='w-full h-full object-cover object-center' src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.collectionName}/${item?.id}/${item?.decor[0]}`} alt='' />
                    </div>
                  </td>
                  <td>
                    <div className='text-center w-16 h-16 flex items-center justify-center rounded-full overflow-hidden'>
                        <img className='w-full h-full object-cover object-center' src={`${process.env.NEXT_PUBLIC_SHOP_API_URL}/files/${item?.expand?.texture_ref?.collectionName}/${item?.expand?.texture_ref?.id}/${item?.expand?.texture_ref?.poster}`} alt='' />
                    </div>
                  </td>
                  <td>
