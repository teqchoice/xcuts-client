...
export default function BlockGallery({ data }: any) {
  // console.log(data)
  // console.log(data[0]?.directus_files_id?.d)
  useEffect(() => {
      ...
      <Slider slidesPerView={4} className={'w-full overflow-hidden'}>
        {data.map((image, index) => {
          ...
          return (
            <Fragment key={index}>
              <a
                href={`${process.env.NEXT_PUBLIC_API__URL_images}/${image?.directus_files_id?.id}/${image?.directus_files_id?.filename_download}`}
                // data-pswp-width={600}
              ...
              >
                <img
                  src={`${process.env.NEXT_PUBLIC_API__URL_images}/${image?.directus_files_id?.id}/${image?.directus_files_id?.filename_download}`}
                  alt=''
